...
markFirst f = updateCoordInField f updatedC
  where
    c = head . head $ f
    updatedC = Coord {row = row c, col = col c, val = val c, ltr = True, rtl = rtl c, ttb = ttb c, btt = btt c}
...
solve x = length . filter (== '#') . concat . fieldToStrings $ process field initialDirs
  where
    field = markFirst . augmentCoordinates $ lines x
    initialDirs = [(head . head $ field, RightD)]
